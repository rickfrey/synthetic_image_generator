...
int main (int argc, char *argv[])
{
    ...
    // TODO:
    // Lichtquelle? (habe ich in modifiziertem Programm (ProofOfConcept) benutzt
    ...
    // Renderer und RederWindow erzeugen
    vtkSmartPointer<vtkRenderer> renderer = vtkSmartPointer<vtkRenderer>::New();
    vtkSmartPointer<vtkRenderWindow> renderWindow = vtkSmartPointer<vtkRenderWindow>::New();
    renderWindow->SetOffScreenRendering(1);

    // Bildgröße anpassen auf verwendetes Endgerät
    renderWindow->SetSize(Rxi,Reta);// Je mehr Pixel desto besser funktioniert HoughLinesP!! Davor: 1200,1599; iPhone 3GS: 1536,2048 (Hochformat)
            ...
            for (float z = zmin; z < zmax; z += Rz) // Schleife über alle z-Werte
            {
                for(int yaw = yawmin; yaw < yawmax; yaw += Ryaw) // Schleife über alle yaw-Werte (yaw muss zuerst definiert werden, da manchmal nicht alle pitch-Posen durchlaufen werden sollen (Decke eher uninteressant))
                {
                    for(int pitch = pitchmin; pitch < pitchmax; pitch += Rpitch) // Schleife über alle pitch-Werte
                    {
                        ...
                        camera->Pitch(pitch);
                        camera->Yaw(yaw);
                        camera->SetViewAngle(FOV); // 63.1 für Galaxy, 49.9 für iPhone 3GS
